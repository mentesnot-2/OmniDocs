...
- [License](#license)
- [Needs improvement](NEEDS_IMPROVEMENT.md)
...
flowchart LR
    ...
    subgraph Data
        DB[(PostgreSQL<br/>users, refresh_sessions,<br/>chat_sessions, chat_messages,<br/>usage_events, support_tickets,<br/>stripe_processed_events)]
...
| Container runtime | Docker + Docker Compose | Defined in Compose files (see `NEEDS_IMPROVEMENT.md` for documenting minimum versions). | `Dockerfile.backend`, `frontend/Dockerfile`, `docker-compose.yml`. |
| Testing | `pytest` | pinned | Test harness in `tests/conftest.py`; suites include `tests/test_auth.py`, `tests/test_file_validation.py`, `tests/test_verification_token.py`, and `tests/test_billing_webhook.py`. |
...
│   │   └── stripe_processed_event.py  # Stripe webhook evt_* idempotency
│   ├── routes/
...
│   ├── test_verification_token.py  # Verification token SHA-256 digest storage
│   ├── test_billing_webhook.py    # Stripe webhook idempotency by event id
...
3. If `EMAIL_VERIFICATION_REQUIRED=true`, a verification secret is generated, **SHA-256 hashed** (`hash_verification_token` in `api/core/security.py`) and stored on `users.verification_token` while only the plaintext secret is emailed; expiry is stored in `verification_expires_at`. If SMTP delivery raises `EmailDeliveryError`, the user row is rolled back and a `503` is returned.
...
sequenceDiagram
    ...
    participant Embed as Cached embedder
    participant Chroma as Chroma tenant store
    participant DB as Postgres

    FE->>API: POST multipart file and CSRF header
    API->>API: CSRF guard and authenticated user
    API->>API: Basename path check and extension allow-list
    API->>API: Monthly upload quota check
    API->>Tmp: Stream body to temp with max byte cap
    Tmp-->>API: total_bytes
    API->>Val: Magic bytes MIME and OOXML checks
    API->>API: Storage quota check for user plan
    API->>Storage: Save file for user id
    API->>Ingest: Parse file then chunk text
    Ingest-->>API: Parsed content and chunk list
    API->>Embed: Batch embed chunk texts
    API->>Chroma: Drop prior vectors then upsert chunks
    API->>DB: Insert upload usage event
    API-->>FE: OK with filename and chunk count
    Note over API,Storage: After a successful save indexing failure deletes the stored object
```
...
- `POST /billing/webhook` verifies signatures with `stripe.Webhook.construct_event`, persists each Stripe `event.id` in `stripe_processed_events` so retries return `{"ok": true, "duplicate": true}` without re-running handlers, then updates `stripe_subscription_id`, `billing_status`, and `plan_id` from `checkout.session.completed`, `customer.subscription.created`/`updated`, and `customer.subscription.deleted`. Errors return generic messages and log details via `logger.exception` to avoid leaking internals.
...
- **Email verification secrets**: outgoing links carry a random plaintext token; `users.verification_token` persists only its SHA-256 hex digest (`hash_verification_token`). `/auth/verify/{token}` hashes the submitted value before lookup (`api/core/security.py`, `api/routes/auth.py`).
...
| Stripe webhook replay / retries | Each `event.id` stored in `stripe_processed_events`; duplicates short-circuit with HTTP 200 so retries do not re-apply billing mutations. |
...
| POST | `/billing/webhook` | Stripe signature | _exempt_ | 120/min | Verifies signature; idempotent on Stripe `evt_*` id (`stripe_processed_events`); updates plan/subscription fields. |
...
| `tests/test_verification_token.py` | `hash_verification_token` stability; signup persists digest; `/auth/verify` accepts plaintext URL token. |
| `tests/test_billing_webhook.py` | Duplicate `evt_*` deliveries ignored; subscription/checkout/delete handlers applied once; unknown event types still record idempotency row. |
| `tests/test_documents.py` | _Placeholder — file is empty._ |
| `tests/test_billing.py` | _Placeholder — file is empty._ |

Coverage reporting is not configured yet; see `NEEDS_IMPROVEMENT.md`.
...
- `users.email` (unique), `users.verification_token` (SHA-256 hex digest of the emailed secret, unique), `users.oauth_sub`, `users.stripe_customer_id` (unique), `users.stripe_subscription_id` (unique), `users.id` (primary).
...
- **No database migrations.** `Base.metadata.create_all` is fine for first-run but does not support iterative schema changes.
...
Current history uses short, imperative-mood subjects (for example “harden auth, storage, and ingestion reliability”). A stricter convention can be adopted per .
